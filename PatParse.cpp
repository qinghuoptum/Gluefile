					...
					LPTSTR pszWorkDir = NULL )
{
	...
	{	// Date&Time Process
		...
		else
		{
			...
			else if ( SubType[0]=='B' )	 // Use mmdd[YY]yy
				ParseDateTime( ps, szResult, 5 );
...
int CPatParse::GetOneRecord( LPTSTR pszTmplt, LPTSTR pszBuf, int nOption, int nLen )
{
	...
	// InitValues( true, false );
	m_bUseNull = (strstr( pszTmplt, "UseNull" )!=NULL);
	...
	if ( pszBuf!=NULL )
	{
		if ( m_szBuf != pszBuf )
		{
			if ( nLen ) 
			{
				...
				strncpy((CHAR*)m_szBuf, pszBuf, MAXRPTLEN);
		}
		// strupr( (CHAR *)m_szBuf);
	...
	else
	{
		...
		if ( nOption == HL7_FORMAT )
		{
			...
			{
				ch = nTmp;
				if ( nState && (nPos <= MAXRPTLEN-5 ) ) m_szBuf[nPos++] = ch;
...
ReScanRec:
				...
				else
				{
					...
					if ( nn>0 && strcmp(szEnds, "*SngRec")!=0 )
					{
						...
						{
							m_szBuf[0] = 0;
							nn = GetDataSegByExtStr(m_pFile, m_stGroup[m_nCurGroup].Delimitor, m_szBuf, MAXRPTLEN);
					...
					{
						nn = fread(m_szBuf,1,MAXRPTLEN,m_pFile);
						if ( nn==0 ) return IMP_REC_END;
						m_szBuf[nn] = 0;
						m_nBLen = nn;
					}
					else
						if (fgets( m_szBuf, MAXRPTLEN, m_pFile )==NULL)
						{	
							m_nBLen = strlen(m_szBuf);
							return IMP_REC_END;
						}
...
int CPatParse::Rmv1stRecord( LPTSTR pszTmplt, LPTSTR pszFile, int nOption )
{
	...
	else
	{
		...
		while ( (nTmp = fread(m_szBuf, 1, MAXRPTLEN-10, m_pFile))>0 )
		...
		while ( (nTmp = fread(m_szBuf, 1, MAXRPTLEN-10, tmpfp))>0 )
...
{
	// CHAR szKTmp[16000]={0}, szVTmp[16000]={0}, szTmp[16000]={0}, szOrgLine[16000]={0}, szLine[16000]={0}, 
	int sizeof_tmp=16000;
	CHAR *szKTmp=new CHAR[sizeof_tmp]; //2014-06-27 for stack overflow
	szKTmp[0]=0;
	CHAR *szVTmp=new CHAR[sizeof_tmp];
	szVTmp[0]=0;
	CHAR *szTmp=new CHAR[sizeof_tmp];
	szTmp[0]=0;
	CHAR *szOrgLine=new CHAR[sizeof_tmp]; 
	szOrgLine[0]=0;
	CHAR *szLine=new CHAR[sizeof_tmp];
	szLine[0]=0;
		
	CHAR ch, *ps, szFormat[1200]={0}, szFmtTpl[1200]={0}, szTmpF[200]={0}, szLE[10]={0};
	//CHAR szBitv[240], szLast[40], szFirst[40], szMid[40], szTitle[40], szCode[20];
	CHAR szSection[100]={0}, szSectSv[100]={0}, szSecBegin[200]={0}, szSecRule[500]={0};
	CHAR szTVal[2048]={0}, szFVal[2048]={0};
	CHAR *ts, *qs, szEachK[240];
	LPTSTR pS, pT, pB, pBSv, pSave, pszNext;
	int nLineNo = 0, nLineNoS = 0, nLineNoT = 0, nSecNo = 0, nPageNo = 0, nPageSize = 0, nn = 0, nLoop = 0, i, j, mm, nL, lcs;
	CTime t1;
	...
	szSection[0]=0;	szSectSv[0]=0;	szSecBegin[0]=0;  
	...
	if ( strncmp( pszTmplt, "RECORDSET", 9 )==0 || strncmp( pszTmplt+1, "RECORDSET", 9 )==0)
	{
		...
		delete[] szKTmp; //2014-06-27 for stack overflow
	    delete[] szVTmp;
	    delete[] szTmp;
	    delete[] szOrgLine; 
	    delete[] szLine;
	...
	if ((ps=strstr( pszTmplt, "SECTIONRULES(*"))!=NULL)
	{
		szSecRule[0]=0;
		ps += 14;
		ts = szSecRule;
		while ( ps[0] && strncmp(ps, "*)", 2)!=0 ) (ts++)[0]=(ps++)[0]; 
		ts[0]=0;
		if ( strncmp(ps, "*)", 2)==0 ) ps+=2;
		while (ps[0]>0 && ps[0]<' ')++ps;
		pszTmplt = ps;

		ps = strstr( szSecRule, "Default:" );
		if ( ps != NULL )
		{
			sscanf( ps+9, "%[^|]", szSection );
	...
	while ( pS[0] && pT-pszBuf<nLen ) 
	{
		...
		{	
			...
			else
			{
				...
				else
				{
					...
					szLine[0]=0;  
					strncpy( szLine, szOrgLine, sizeof_tmp-1 );
					...
					if ( szSecRule[0] )  // Looking for Section Name
					{
						...
						if ( ( (strcmp( szSection, szSectSv ))!=0) && szSection[0] 
							    && strcmp( szSection, "Takeoff") ) 
						{
							nLineNoS = 1;
							nSecNo++;
							strcpy( szSecBegin, szSection );
						}
						else 
							szSecBegin[0]=0;
						
						SetOneValueEx( "SectionName", szSection );
						SetOneValueEx( "SectionBeg", szSecBegin );
		...
		else if ( pS[0]=='<' && pS[1]=='*' ) 
		{
			...
			if (pS[0]=='>')	 // Get a Variable Item
			{
				...
				{
					szVTmp[0] = 0; szKTmp[5]='#';
					SetAllText(szKTmp+5, szVTmp, sizeof_tmp, (pszBody!=NULL?szLine:NULL));
				...
				{
					// Repeated by a field
					GetOneValue( szKTmp+5, szTmp, sizeof_tmp );
					ch=0; szVTmp[0]=0; 
...
Continue:						
				...
				else if ( strncmp(szKTmp,"TextField", 9)==0 && pB != NULL )
				{
					...
					if ( (strcmp( szSection, szSectSv ))!=0 && szSection[0] ) strcpy( szSecBegin, szSection );
					else szSecBegin[0]=0;
				...
				else
				{
					...
					szKTmp[0]='#';
					SetAllText(szKTmp, szVTmp, sizeof_tmp, (pszBody!=NULL?szLine:NULL));
					strncpy(szKTmp, szVTmp, sizeof_tmp);
					...
					if ( szKTmp[0]=='?' )
					{
						...
						while ( ts[0] )
						{
							...
							nL = GetProperKeyVal( szEachK, szVTmp, sizeof_tmp-1, szLine, nLineNo, nLineNoT, pB!=NULL );
							// i = strlen( szVTmp )-1;
					...
					else
					{
						...
						else if ( strcmp(szKTmp,"LineNoS")==0 && pB != NULL )
						{
							if (pB[0])
								sprintf( szVTmp, "%d", nLineNoS );
							else
							{
								ps = NULL; szVTmp[0]=0;
							}
						}
						else if ( strcmp(szKTmp,"SecNo")==0 && pB != NULL )
						{
							if (pB[0])
								sprintf( szVTmp, "%d", nSecNo );
							else
							{
								ps = NULL; szVTmp[0]=0;
							}
							...
							nL = GetProperKeyVal( szKTmp, szVTmp, sizeof_tmp-1, szLine, nLineNo, nLineNoT, pB!=NULL );
					}
					...
					if ( szFormat[0] )
					{
						...
						SetAllText(szFmtTpl, szTmp, sizeof_tmp-1, (pszBody!=NULL?szLine:NULL));
...
void CPatParse::SetOneValue( LPTSTR pszKeyName, LPTSTR pszKeyValue, int nOver )
{
	...
	if ( m_bUseNull && pszKeyValue[0]==0 ) strcpy( pszKeyValue, "NULL" );
...
/* 0:Normal Name, 1: Last, 2:X., 3:X.X. ..., 4:Jr. ..., 5: Code, 6: Other */
	...
	while ( ps[0] )
	{
		...
		// if ( ps[0]=='[' || ps[0]=='(' || ps[0]=='/' ) i = 0;
	    if ( ps[0]=='[' || ps[0]=='(' ) i = 0;
		if ( ps[0]=='[' || ps[0]==']' || ps[0]=='(' || ps[0]==')' || ps[0]=='/' ) ps[0]=' ';
		++ps;
	}
	DCode[i]=0;
...
/* 0:Normal Name, 1: Last, 2:X., 3:X.X. ..., 4:Jr. ..., 5: Code, 6: Other */
  ...
  if ((p>=3) && cbuf[0]=='R' && ((cbuf[1]=='-') || (cbuf[1]=='#'))) return 3;
...
// nDMY=5: mmddyy, nDMY=4: yyyy/mm/dd, nDMY=1: dd/mm/yyyy, nDMY=2: Age, nDMY=3: use system time;
...
long ParseDateTime(char *inBuf, char *outBuf, int nDMY )
{
	...
	else if (strspn(ps1,"0123456789")==6)
	{
		...
		else if (strncmp(ps1,"00",2)==0 || strncmp(ps1,"13",2)>0)
		{
	...
	else if (strspn(ps1,"0123456789")==8)
	{
		...
		else if (strncmp(ps1,"20",2)==0 || strncmp(ps1,"19",2)==0)
		{
...
// _Ti_ ::= F<n>|#<n>-<m>|<Keyword>|$<Keyword>|Default:
// F:<n> ::= First <n> lines (including blank line)
// #<n>-<m> ::= Line range <n>-<m>
// $<Keyword> start with this Keyword
//  
...
bool StdRngExprPrs( LPTSTR pzTemplt, LPTSTR pzVar, int nOpt, LPTSTR pzRes, int nLmt )
{
	...
	bool ss = false, ns = false, bSng, bCur;
	int nn, np=0, nl=0;
...
//#endif
	...
	while ( ps[0] )
	{
		...
		else if ( (nOpt & STD_RNGE_CHECK) || (nOpt & STD_RNGE_ENUM) )
		{
			if ( szLwr[0]=='~' )
			{
				ns = true; strcpy( szTmp, szLwr+1); strcpy( szLwr, szTmp);
			}
