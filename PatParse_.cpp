...
bool ExtendEvaluate( LPTSTR pzOrgExpr, LPTSTR pzResult )
{
	...
	while ( ps[0] )
	{
		ts[++i]=ps;
		qs = strchr(ps,'~');
		if (qs==NULL) qs = strchr(ps,':'); 
		if (qs==NULL) break; // { ts[i]=(ps-1); break; }
		else { qs[0]=0; ps = ++qs; }
	}
	ts[++i]=ps;
	...
	if ( expr.Evaluate( pzOrgExpr, szTmp ) )
	{
		...
		if (strcmp(szTmp,"TRUE")==0) strcpy( pzResult, ts[1] );
		else if (strcmp(szTmp,"FALSE")==0) strcpy( pzResult, ts[2] );
...
{
	int i, nn, nBuf, ss, n1, n2, n3, nSv, k;
...
//		if ( nn==8 && (strcmp(szEvent,"000")==0 || strstr( (LPTSTR)m_szBuf, szEvent )!= NULL) )
		if ( nn==8 && CheckHL7Event((LPTSTR)m_szBuf, szEvent) )
		{
		  ...
		  strcpy(tmpSegment, szSegment);
		  strcat(tmpSegment, szMark2);

		  while ( ((pS1 = strstr( pos, tmpSegment)) !=NULL) || strcmp(szSegment,"XXX")==0 )  
		  {
			++nSegC;
...
When using the these values, if they start with #, not update this field outside, 
...
{
	CHAR szKTmp[4000], szVTmp[4000], szTmp[4000], szOrgLine[4000], szLine[4000], ch;
	CHAR *ps, szFormat[1200], szFmtTpl[1200], szTmpF[200], szLE[10];
	//CHAR szBitv[240], szLast[40], szFirst[40], szMid[40], szTitle[40], szCode[20];
	CHAR szSection[40], szSectSv[40], szSecRule[250];
	CHAR szTVal[2048], szFVal[2048];
	...
	CTime t1;
	BOOL sRes, ss, bDisc = true, bBin = false;
	int nTmp;
...
bool CPatParse::SelectPropItem( LPTSTR pszTmplt, LPTSTR pszBuf, int nLen )
{
	char szTmplt[4096]={0}, szRes[512];
	char szCriteria[512]={0}, szItem[512]={0};
	char *ps, *qs, ch;
	int nn = 0;
	bool ss = false;
	...
	while ( ps[0] )
	{
		...
		SetAllText( szCriteria, szRes, 512 );
		if ( strstr( szRes, "TRUE" )!=NULL ) { ss = true; break; }
	}

	if ( ss ) strncpy( pszBuf, szItem, nLen );
	return ss;
}
...
{
	char *ps;
	char szTmp1[30], szTmp2[20], ch1, ch2;
	int nPos =1, m, nL;
	...
	if ( nOption == 0 )  // Text String Format: <FieldName>=<len>|...
	{
		...
		while ( ps[0] )
		{
			...
			sscanf(ps,"%20[^=]%c%[^|]%c", szTmp1, &ch1, szTmp2, &ch2);
...
// nDMY=1: dd/mm/yyyy, nDMY=2: return the age, nDMY=3: use system time;
// Format: mm/dd/yyyy|yyyymmddHHMMSS|mm-dd-yyyy|Mon dd, yy|dd-Mon-yy|
//
long ParseDateTime(char *inBuf, char *outBuf, int nDMY )
{
	char *ps1, *ps2, szTemp[30], s1[30], s2[30], s3[30], s4[30], s5[30], ch;
	int  k,d1,d2,d3,t1,t2;
	long nTime;
	...
	{
		ch = 0;
		sscanf(ps1,"%[^:]%*c%[0-9]%*[^a-zA-Z]%c", s4, s5, &ch);
		if ( ch=='a' || ch=='A' )
		{
...
int GetCTimeStr( char* pszBuf, char* pszFmt, char* pszVal )
{
	...
	struct tm x;
	char *ps, ch, szTmp[50], szT0[10], szT1[10], szT2[10], szT3[10], szT4[10], szT5[10];
	time_t t = 0;
	...
	else
	{
		...
		strcpy( szTmp, ps );
		//int i,k;
		//for (i=0,k=0; pszVal[i] && (i<50-1); i++)
		//	if (pszVal[i]>' ') szTmp[k++]=pszVal[i];
		//szTmp[k]=0;
	}
